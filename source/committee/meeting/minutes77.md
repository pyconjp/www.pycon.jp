# 2025年12月8日(月) 一般社団法人PyCon JP Association運営会議#77

* 日時: 2025年12月8日(月) 19:30-21:00
* 場所: CMSコミュニケーションズ、Zoom
* 参加者 <https://pyconjp-staff.connpass.com/event/364988/>
  * 理事: takanory, shimizukawa, terada, jonas, yoshida, maaya
  * オブザーバー: ike, sano, tsutui, nishimoto, tsukamoto, yamage

```{contents}
:local: true
```

## PyCon JP

### PyCon JP 2025(nishimotz、高、報告)

* 主催メンバー
  * 旅費、宿泊費の支援を実施
    * 精算を行った
  * チームの終了やアカウント停止などを進める（年内を目標）
* [アンケートまとめを公開](https://pyconjp.blogspot.com/2025/11/pyconjp2025-questionnaire.html)
* gihyo.jp 記事
  * [https://gihyo.jp/article/2025/10/pycon-jp-2025-report](https://gihyo.jp/article/2025/10/pycon-jp-2025-report)
* 作業会
  * 12月9日(21:00-)に行う
  * [PyCon JP 2025 主催メンバー定例会/作業日 #37 - connpass](https://pyconjp-staff.connpass.com/event/377917/)
* 打ち上げなど
  * [東京での打ち上げを行った](https://pyconjp-staff.connpass.com/event/373756/)
* CoCチーム
  * Maaya, nyoho, terada
  * 解散しました(terada
* 会計
  * [予算2025](https://docs.google.com/spreadsheets/d/1_RsUdRouEV21vkqpbdkTeOeqMeDk83wgpMI7IkxPJFQ/edit?gid=1143679075#gid=1143679075)
  * 広島コンベンションビューローの助成金300万円、託児実施の助成金3万円
    * 請求手続きなどを進める
  * 精算や支払いを進め、PyCon JP 2025として会計を締める
* 課題 
  * KYTさんへの支払いが遅延している。10月末日支払い期日の請求書に対し、12月4日時点で未払いの連絡をKYT殿より受信して認識(Makino
    * 12月7日に緊急支払いした(terada
  * 動画アーカイブ
    * 納品映像チェックを進める
	* 2024では下記のスクリプトで登録を行った
    * <https://github.com/pyconjp/youtube-video-setting> (terada
  * Issue 整理
* 質疑応答
  * sano) 会計の託児の助成金5万円については申請済みで、口座の確認をお願いしたい
    * 5万円の入金を確認した(shimizukawa
  * takanory) メインの助成金がどうなったか少し心配
    * nishimotz) 情報は揃っているので進める
    * shimizukawa) 300万円は仮払金となるかも?ということで認識した
  * takanory) 困っていることはありますか?
    * nishimotz) とくにはない
    * terada) 動画公開は2024ではteradaとpeacockでツールを作ってやっていた。情報が揃えば公開は可能

### PyCon JPの開催の最低要件(terada、低、報告)

* 最終版とした: [PyCon JPの構成要素(案)ISSHA-3491](https://docs.google.com/spreadsheets/d/131WSpp5mhy1JzdFcZOPofc3L-Zt3-CwzaGOC5erzNQQ/edit?gid=0#gid=0)
* PyCon JP 2025が始まるときにイベントの最低要件をterada、takanory、yoshida中心にまとめて作成した。最終版としてまとめた
* これを2026チームに一旦お渡ししました(terada
  * 受け取ったが、運用はちょっと考えられていない。MUSTのものがそのままMUSTでいいのか?とかは将来的に見直したい(takanory
  * 公開しますか？してもいいと思うけどMUSTに縛られると動きづらくなるのでそれは避けたい気持ちがありますね(shimizukawa

### PyCon JP 2026

#### PyCon JP 2026(sano、takanory、中、報告)

* 会場
  * 広島国際会議場
* 会場利用申請
  * 2026年8月20日(木) 午後から準備、ヒマワリ（2025スポンサーブース）のみ18時から
  * 2026年8月21日(金)、22日(土) 全館
    * 全館を借りていると廊下が結構自由に使えるはず(terada
  * 2026年8月23日(日) 会議運営事務室、ヒマワリ
* PyCon JP 2026のJIRAスペースを作った
  * [ISSHA-3768: PyCon JP 2026用JIRAの準備](https://pyconjp.atlassian.net/browse/ISSHA-3768)
  * <https://pyconjp.atlassian.net/jira/core/projects/HRS/board?filter=&groupBy=status>
  * 新規に作ったらカテゴリの設定方法がわからない...
* [HRS-4: PyCon JP 2026の方向性まとめ](https://pyconjp.atlassian.net/browse/HRS-4)
  * スプシでやること、やらないことを擦り合わせ中
  * [PyCon JP 2026でやりたいこと、やらないこと、維持するとこ](https://docs.google.com/spreadsheets/d/1nvjmebcVOhvXQWcK4V29Qu3mRijRhjQLhEHhJdpBmlE/edit?gid=0#gid=0)
* 11月26日にキックオフミーティングを実施
  * [PyCon JP 2026 キックオフミーティングを開催しました](https://pyconjp.blogspot.com/2025/12/pycon-jp-2026.html)
  * 自己紹介と、上記スプシの方向性すりあわせを途中まで
  * 引き続きやっていく
    * 次回は12/9 19:00 -> <https://pyconjp-staff.connpass.com/event/378192/>
* 質疑応答
  * いま何人くらいで動いてますか？(shimizukawa
    * 登録が22名(sano
    * とはいえまだキックオフをしただけなので、これからどうなるか不確定(takanory
  * 佐野さん（座長）は、一社の運営メンバーと同等の権限を渡すルールが良いのでは？(terada
    * 一社側のドキュメントが見れないため不便(terada
    * 今、一時的に権限を佐野さんにだけ設定している。どう設定するのがいいか悩む。提案した方がいいかなと思った(takanory
    * よさそう。共同座長だけじゃなくてチームリーダーとか(terada
    * **TODO**: ルール考えてPyCon JP 2026側から提案する(sano, takanory
    * **TODO**: 運営メンバーになって運営メンバーページに載せるのがいいのではないか(terada
      * それはよさそう +1(takanory, shimizukawa

#### PyCon JPの一般社団法人とイベントチームの役割分担について(terada、高、相談

* [FYI] JJUGではコミュニティ(今期から一般社団法人) 事務すべてをこの会社に委託してやってもらってます(maaya
  * [ISSHA-3764: イーサイドとのミーティング実施](https://pyconjp.atlassian.net/browse/ISSHA-3764)
  * [株式会社イーサイド](https://www.e-side.co.jp/) 
  * 契約周りやお金周りの事務をやってもらっている。
  * 数年間カンファレンスの周りもやってもらっている。
  * ロジスティック周りもやっている
  * 11月25日に打ち合わせ済み [20251125 イーサイドと打合せ](https://docs.google.com/document/d/1REWGnBOupc8XrKH8kEW1fzC36pPTLF-KtgiGpg8j1aU/edit?tab=t.0#heading=h.xxxau7vd5pvc)
    * いろいろお願いできそうだった(takanory
    * PyCon JP 2026チームとして予算をつけて依頼することを検討するつもり(takanory
      * 具体的な内容はまだこれからです
    * 一社としても依頼する?(takanory
  * イーサイドさんと話をして、部分的な依頼ができるのではないかと思った
    * イベントでは、会場や飲食とか、スポンサーとか
    * 一社側でも必要があれば、お願いできる部分もありそう。
    * 一社側だと細かくいろいろあるので、難しそう(shimizukawa
    * 最初の馬力が要りますね（全員

## 会計関連

### 2026年仮予算(takanory、高、相談)

* 参考: [2024年12月16日(月) 一般社団法人PyCon JP Association運営会議#70](https://www.pycon.jp/committee/meeting/minutes70.html#id3)
* **合計: 1,370,000円**
  * PyCamp Caravan
    * 2026/2/27(金)・28(土) OSC東京春 50,000円 (yoshi-tsukamo
  * PyLadies Caravan in 福岡 2nd 分の予算をお願いします 100,000円 (maaya
  * PyCon JP TV 懇親会費(懇親会費 x 3) 30,000円 (terada
  * PyCon mini Shizuoka
    * イベント支援 50,000円
    * イベント中止保険（1万円？）
    * ピザ支援（パーティーを予定しているため）(yoshi-tsukamo
  * 日本からスタッフのみなさんへのお土産（PythonAsia） 3000円 (terada
  * 会計事務所（1,2月分） 100,000円 (shimizukawa
  * 会計事務所（決算書作成等の費用） 310,000円
  * 行政書士（総会）: 50,000円
  * ​​情報セキュリティ指針コンサル費用: 30,000円
  * インフラ（AWS 1,2月分） 10,000円
  * 消費税: 不明（50万～60万くらい？）
  * レンタルオフィス: 1万円（初期費5500円+1000円/月）
  * 予備費 : 50,000円（例年通り
* 質疑応答
  * レンタル倉庫は？(takanory
    * 3月契約で考えているので仮予算外でOKです(shimizukawa
* 理事の合意
  * 賛成(terada, jonas, maaya, shimizukawa, yoshida, takanory

### 2025年 遠方支援(shimizukawa、低、共有)（12/8更新）

* **TODO:** 感想をblogにする（2025年内）
  * 執筆中。
  * 12月中に公開を目指して、今週12/8週中にレビュー依頼します
* 次回に向けてメモ
  * 遠方支援でなにかする必要があるか？と10人以上ヘルプデスクに来た。来年以降も同じ方針にするのであれば案内が必要そう(yoshida
  * メールには「当日どうしてね」は書いていなかったので、案内はあった方がいいかも(shimizukawa
  * 今後は、遠方支援のルールのFAQを用意するとよさそう(shimizukawa

## PyCamp、PyLadies関連

### PyCamp状況報告(ryu22e、低、報告)

* 運営メンバー: ryu22e、yamate、nishimoto、kobatomo
* [Python Boot Camp(初心者向けPythonチュートリアル) — PyCon JP](https://www.pycon.jp/support/bootcamp.html)
* 2025年12月以降の開催見込み
  * 長崎（現地スタッフ: 平山さん）
    * 平山さんはryu22eの同僚
    * 4月のオンライン相談会にも来てくれた
    * **佐世保**で開催したいとのこと
    * 会場はこれから探す
  * 福岡県北九州市（現地スタッフ: 日下部さん）
    * 12/20（土）開催 [https://pyconjp.connpass.com/event/376015/](https://pyconjp.connpass.com/event/376015/)
    * 講師：ryu22e
    * 担当コアスタッフ：kobatomo
  * 山口県周南市（しゅうなんし）（現地スタッフ: 中嶋さん）
    * 今仕事が忙しいので、動けるのは2026年4月以降とのこと
    * 現地スタッフ受け入れ手続きは一旦ストップしていて、4月以降に進める予定
* 質疑応答
  * 山口県周南市は北九州市から近いので、中嶋さんにお手伝いor参加の声をかけてみるのはどうでしょう？雰囲気も分かると思うので(terada
    * メールしてみます(ryu22e

### OSC出展(Python Boot Camp Caravan)(yoshi-tsukamoto、中、報告)

* 運営メンバー: yoshi-tsukamo
* 出展済み
  * [OSC2025 福岡](https://event.ospn.jp/osc2025-fukuoka/)
    * 2025/11/22(土)
    * [九州産業大学](https://www.kyusan-u.ac.jp/) 12号館
    * takanoryさん参加
  * 今年の出展を振り返るミーティングを予定
    * 来年やること（ブースで出すもの）を検討したい
    * 今年のOSC参加者に限らず参加してもらいたい
* 来年のOSC
  * 2026/1/31(土) 大阪（行かない予定）
    * 大阪産業創造館
    * [https://event.ospn.jp/osc2026-osaka/](https://event.ospn.jp/osc2026-osaka/)
    * 12/15受付締切
  * 2026/2/27(金)・28(土) 東京春
    * 駒澤大学
* 仮予算
  * OSC東京春 50,000円でお願いします（2名参加を想定）

### PyLadies関係報告(maaya, 中, 報告)

* PyLadies Caravan
  * PyLadies Caravan in 静岡 12月13日
    * 地元スタッフ：mizukiちゃん、yoluさん (サポート佐野さん)
    * データクレンジングと可視化のハンズオン
    * 場所確定済み([静岡市 コ・クリエーションスペース](https://coc-shizuoka.jp/about/))
  * PyLadies Caravan in 福岡 2nd 2/28
    * 地元スタッフ：みゆきさん、Ziiさん
      * みゆきさんcaravan広島＆PyCon JP2025で直接MTG済み、ZiiさんPyCon JP 2025でリアルご挨拶済み
    * 福岡市内開催予定で場所選定中
      * 予算より高い金額で予約
      * kanan分交通費がかからないので結果予算内
  * 金沢大学の学生さんからcaravanやりたい旨お問い合わせあり
    * 大学入試関係で冬は大学が使えない＆雪で大学にたどり着けないとのことなので春に開催の方向で調整中
    * PyCon JP 2025 でリアルご挨拶済み

* その他PyLadies
  * [PyLadiesCon](https://2025.conference.pyladies.com/en/) 今年もありました(12/6)

### PyCon JP TV(terada、低、報告)

* パーソナリティー: takanory, terada
* 運営メンバー: peacock、nana
* [https://www.youtube.com/user/PyConJP](https://www.youtube.com/user/PyConJP)
* Web [https://tv.pycon.jp/](https://tv.pycon.jp/)
* 59回配信済み
  * [#59: 生成AIを使ったコーディングのライブデモ: Streamlitでフロント実装編 - 2025-12-05](https://tv.pycon.jp/episode/59.html)
  * [#58: 地域コミュニティ座談会 - 2025-11-07](https://tv.pycon.jp/episode/58.html)
* 次回は、1月9日（金）予定
  * [#60: Pythonistaに聞く2025重大ニュースと2026の展望 - 2026-01-09](https://tv.pycon.jp/episode/60.html)
* 今年の目標であった地方からの放送を実施した。
  * #58: 地域コミュニティ座談会 - 2025-11-07
* ネタ募集中です(takanory
  * [開発系とイベント紹介系を軸に進めていくつもり](https://docs.google.com/spreadsheets/d/1N7QVU9uTcZeoHCrnzWw0LLZQ5lb82ocHopCbUO5biwo/edit#gid=0)
  * 特に技術ネタのアイデアが欲しい
* 質疑応答、コメント
  * 出張放送はまだ1回しかできていないので、また開催して練度を上げていきたい(takanory
    * 2月静岡かな(ボソ

## コミュニティー支援

### PyCon mini 東海 2025（shimizukawa、低、報告）

* [ISSHA-3534](https://pyconjp.atlassian.net/browse/ISSHA-3534)
* 日程: 2025/11/08 開催済み✅️
* 会場 [ISSHA-3572](https://pyconjp.atlassian.net/browse/ISSHA-3572) 済み: 5/下旬 契約、支払い済み✅️
* Zoom貸し出し [ISSHA-3574](https://pyconjp.atlassian.net/browse/ISSHA-3574) 済み✅️
* 開催Blog: [ISSHA-3575](https://pyconjp.atlassian.net/browse/ISSHA-3575) : 済み✅️
* 保険契約:[ISSHA-3576](https://pyconjp.atlassian.net/browse/ISSHA-3576) : 見積り5200円、11月頭に済み✅️
* メディア告知: [ISSHA-3577](https://pyconjp.atlassian.net/browse/ISSHA-3577) : 済み✅️
* 費用精算: [ISSHA-3773](https://pyconjp.atlassian.net/browse/ISSHA-3773)  11/19 済み✅️
* 開催報告blog: 未🌀

### SciPyDataJapan2026 広報支援（shimizukawa、低、報告）

* [ISSHA-3665](https://pyconjp.atlassian.net/browse/ISSHA-3665)  （理事で合意し手続きを進行中）
* [https://scipydata.connpass.com/event/364718/](https://scipydata.connpass.com/event/364718/)
* 日程: 2026年1月24日(土)
* 会場: 御茶ノ水ソラシティ
* Blog: アカウント付与済み、作成済み✅️
* メディア告知: 済み✅️
* 開催報告blog 未🌀

### 貸出物品に小物を出せるか検討 (shimizukawa、高、相談)

* [https://pyconjp.atlassian.net/browse/ISSHA-3501](https://pyconjp.atlassian.net/browse/ISSHA-3501)
* [コミュニティに貸出可能な物品](https://www.pycon.jp/support/goods.html)があるのですが、大きいモノしかない & 配送に課題がありそうです。
* 小物類を貸出物品に含められるか検討したいと思います。
* 2024/12/16時点
  * あるとうれしいものはスタッフ腕章、ペン、電源タップ、変換アダプタあたり?(terada
  * 倉庫サービスを契約して、そこから直接送付・受取するのはどうか?(takanory
    * [段ボール荷物預かり保管サービス6社を比較｜料金相場やメリットについても解説します - 倉庫シェアリングならsouco](https://www.souco.space/borrower/cardboard/column/storage-service-comparison/)
  * 東海、静岡の意見
    * 宮崎: 1万円くらいかかった。年一で持っておくより、地域イベントで回せたら良さそう
    * 塚本: あると助かる
* 2025/12/8時点
  * 東海から2回目開催後のフィードバックをもらいました
    * 当日のみ使用する消耗品 (名札用のペンなど) の貸し出し。ただし、昨年と比べて東海で持っている消耗品・備品の在庫も増えてきたので重要度は下がりました。
  * しかし、[ISSHA-3694](https://pyconjp.atlassian.net/browse/ISSHA-3694) で検討した結果、一社の荷物を整理して貸し出せるものが無くなる予定です（倉庫サービスの継続的な保管料より、都度買うほうがムダは多いが経済的）
  * ということで、以下の方針で進めようと思いますがどうでしょうか。’
    * 小物貸し出しの検討は終了
    * [コミュニティに貸出可能な物品](https://www.pycon.jp/support/goods.html) ページを削除
  * 議論、ご意見
    * 致し方ない、この方向しかないと思う(takanory
    * 物品をどこかのコミュニティーに引き取ってもらう検討をしているが、そこが必要なときに貸してくれる可能性もあるかもしれない、が、可能性は低そう(takanory
    * 
  * TODO: 
    * [コミュニティに貸出可能な物品](https://www.pycon.jp/support/goods.html) ページを削除を進めます(shimizukawa
  * この議題は今回で終了

## 海外コミュニティ連携

### PAO(APAC)関連(terada、低、報告)

* PAO (Python Asia Organization) の活動が始まっている。
  * [PyCon JP Blog: PythonAsia (カンファレンス) の紹介とOnline Charity Talk H2イベントの紹介](https://pyconjp.blogspot.com/2025/12/pythonasia-online-charity-talk-h2.html)
  * PythonAsia（カンファレンス）は3月にフィリピンで開催
  * オンラインイベントを1月末に開催
* 一社PyCon JP AssociationとしてPAOに協力できることなどのアイデアがあれば知りたい(takanory
  * Python PH(フィリピン)はSilverになる。
  * PyCon JP Associationとしてもスポンサーしてもらえるとうれしい(terada
  * TWも積極的に検討中
  * <https://github.com/PythonAsiaOrganization/public-docs/blob/main/sponsorship.md>
    * Silver 600 Euro

### 海外イベント参加予定

* この後は、Indonesia / PythonAsia(Philippines) / PyCon USと続く
* 日本からスタッフのみなさんへのお土産が持って行くという動きが、アジア内で多く見かけるようになり一般化してきている。
  * 【決定】来年度は初期から予算を決めて3000円*10回とか実施する
  * 1回目は仮予算として3000円が決定している。

## その他

### 運営メンバー (terada、 中、報告)

* Ikedaさん、Makinoさんが運営メンバーとなりました。
* 座長などが運営メンバーになる方向という議論が別で進んでいるので、方向性としては良いと思う(nishimotz

### PyCon JP共有インフラについて(yoshida、 中、報告)

* OSが古くなってきているため、更新する必要がある。
  * [https://pyconjp.atlassian.net/browse/ISSHA-2482](https://pyconjp.atlassian.net/browse/ISSHA-2482)
  * Debian GNU/Linux 13 に更新した。
  * 固定IPエラスティックIPにはできてない
    * AWSメンテナンスでIPが変わる可能性がある
    * [https://pyconjp.atlassian.net/browse/ISSHA-3529](https://pyconjp.atlassian.net/browse/ISSHA-3529)
    * 変更するとconnpassのAPI制限の再申請が必要
    * ↑connpass API v2ではAPIキーがあればよいので、固定IPが不要になった。APIキーは1passwordに書いてある(takanory
* 現状、2024以降はダイナミックコンテンツのママ
  * [https://pyconjp.atlassian.net/browse/ISSHA-3530](https://pyconjp.atlassian.net/browse/ISSHA-3530)
* pyconjpbot
  * 暫定動作
  * [https://pyconjp.atlassian.net/browse/ISSHA-3528](https://pyconjp.atlassian.net/browse/ISSHA-3528)
    * Pythonバージョンもやばいので、全面刷新しないといけない...(takanory
* 古いインスタンスについて
  * [https://pyconjp.atlassian.net/browse/ISSHA-3529](https://pyconjp.atlassian.net/browse/ISSHA-3529)
  * インスタンス実行で費用がかかっているので、停止した
* jira-issue-reportがpycon.jpサーバー上で動いてなさそう。調査する(takanory
  * [https://pyconjp.slack.com/archives/C024JGVAU/p1760338459882639](https://pyconjp.slack.com/archives/C024JGVAU/p1760338459882639)
  * ↑対応済み(takanory

### 情報セキュリティ指針や情報レベルの定義  (terada、 中、議論)

* <https://pyconjp.atlassian.net/browse/ISSHA-3638>
* 松山さんという専門家に業務委託をして、一緒に月2回ペースでMTGを実施して進めている
* Ikeさんにも参加いただき、現在は、teradaとtakanoryも入って3名で対応している
* 最初の対策方針案を作ってレビュー中
  * [情報セキュリティ対策のための方針](https://docs.google.com/document/d/1qjlORbB3kt4nyyoIK9CA23N4zAXBYhBRXG96l8_dKvE/edit?tab=t.0#heading=h.8n1tqv9kmmjt)
  * 本日、この合意をしたい。
  * 監査ログのところを除いて合意できないか?(terada
* 質疑応答
  * Google Workspaceのユーザー数上限への影響は？(yoshida
    * 心配ない(takanory
  * 二要素認証の強制も一般化してきているので問題ないと思う(yoshida
  * 自動化に影響がでるかもしれないが、APIキーでなんとかななりそうかな(yoshida
  * Boot camp でJiraを使う人向けにメアドを発行する必要が出てくる？(yamate
    * 確かに。考えます(terada
    * 新しい指針で考えると、pycamp関連のタスクはjira以外にする必要があるかも(takanory
    * Jiraに機密情報は入れたい(terada
    * Jiraのプロジェクトを分けて運用できるかもしれない(takanory
      * GitHub Issueに置くのもありかもしれない(terada
    * 別案、JIraに重要な情報を入れない、重要情報はすべてGoogle Driveに逃がす(jonas
      * それは難しいかもーー(takanory
  * 監査ログについて、Google Workspaceからのメールを記録する仕組みがあると念の為、良いかもと思いました(shimizukawa
    * もしそうなったら私はそのメールを使いたくない(terada
    * メールを誰でも見れるようにするのではなく、監査に必須なときだけ特定の権限を持つ人だけが見えるようにするのはあり？(ike
    * それなら良いと思う(terada
    * [Google Vault](https://support.google.com/vault/answer/2462365?hl=ja) というサービスがあるようです(yoshida
  * 監査ログで中身を見るというところはあるので、慎重に判断した方がいいと思う。反発を受けるので。あとはシステム管理者がなにをできるかも決めた方がよい(ike
  * **TODO**: この件だけ合意する会を別途やった方がよさそう(takanory

### 寺田が理事から離れる準備  (takanory、 中、報告)

* [ISSHA-3691: 寺田さんが理事から離れることに伴い登記先などを調査して方針を決める](https://pyconjp.atlassian.net/browse/ISSHA-3691)
* takanory, yoshida, shimizukawa で調査してミーティングで情報共有をしている
* バーチャルオフィスを使用する方向→[バーチャルオフィス1](https://virtualoffice1.jp/)に決定
  * [ISSHA-3797: バーチャルオフィス1申し込み](https://pyconjp.atlassian.net/browse/ISSHA-3797) 
* 荷物はトランクルームを使う想定→[minikura（ミニクラ](https://minikura.com/)に決定
  * [ISSHA-3804: トランクルームの確定と契約](https://pyconjp.atlassian.net/browse/ISSHA-3804)
* 検討事項
  * 郵便物などの受取→会計周りの書類は羽藤会計事務所に直送。それ以外はバーチャルオフィス1に送る予定

## 次回

* 運営会議#78
  * 2026年1月19日(月)
    * 決算の準備
    * 総会の準備
    * 理事募集しないと
* 総会 & 運営会議#79
  * 2026年2月26日(木)
